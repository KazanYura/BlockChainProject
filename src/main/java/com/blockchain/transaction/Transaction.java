...
public class Transaction {
    ...
    private String calulateHash() {
        ...
        return Hasher.applySha256(
                Hasher.getStringFromKey(sender) +
                        Hasher.getStringFromKey(reciepient) +
                        Float.toString(value) + sequence
        );
    }
    //Signs all the data we dont wish to be tampered with.
    public void generateSignature(PrivateKey privateKey) {
        String data = Hasher.getStringFromKey(sender) + Hasher.getStringFromKey(reciepient) + Float.toString(value)	;
        signature = Hasher.applyECDSASig(privateKey,data);
    }
    //Verifies the data we signed hasnt been tampered with
    public boolean verifiySignature() {
        String data = Hasher.getStringFromKey(sender) + Hasher.getStringFromKey(reciepient) + Float.toString(value)	;
        return Hasher.verifyECDSASig(sender, data, signature);
    }
